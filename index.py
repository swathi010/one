...
def Exit():
    result = tkMessageBox.askquestion('Inventory Monitoring System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Inventory Monitoring System/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("Inventory Monitoring System/Home")
    width = 1024
    height = 520
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Inventory Monitoring System", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    filemenu2.add_command(label="Create New", command=ShowAddNew)
    filemenu2.add_command(label="Inventory List", command=ShowView)
    menubar.add_cascade(label="Account", menu=filemenu)
    menubar.add_cascade(label="Inventory", menu=filemenu2)
    Home.config(menu=menubar)
    Home.config(bg="yellow")

def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("Inventory Monitoring System/Create New")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def AddNewForm():
    ...
    lbl_text = Label(TopAddNew, text="Create New Item", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
    ...
    lbl_price = Label(MidAddNew, text="Price:", font=('arial', 25), bd=10)
...
def ViewForm():
    ...
    lbl_text = Label(TopViewForm, text="Inventory List", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Inventory Monitoring System', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Inventory Monitoring System/Inventory List")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Inventory Monitoring System', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
